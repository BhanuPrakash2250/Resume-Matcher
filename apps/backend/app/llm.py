...
def resolve_api_key(stored: dict, provider: str) -> str:
    """Resolve the effective API key without cross-provider key leakage."""
    ...
    provider_key = str(api_keys.get(config_provider, "") or "").strip()

    # Groq must never inherit a stale top-level OpenAI key. Prefer the
    # provider-specific Groq key when it is present.
    if provider == "groq":
        if provider_key:
            return provider_key
        # settings.llm_api_key is the application's environment-backed
        # fallback and is used only when no provider-specific key exists.
...
def get_llm_config() -> LLMConfig:
    ...
    # Groq GPT-OSS accepts only low/medium/high. Treat any other configured
    # value (especially the OpenAI-specific "minimal") as unset.
    if (provider or "").strip().lower() == "groq" and reasoning_effort not in {None, "low", "medium", "high"}:
...
def get_model_name(config: LLMConfig) -> str:
    """Return the exact LiteLLM model identifier for a provider."""
    provider = (config.provider or "").strip().lower()
    model = (config.model or "").strip()

    if provider == "groq":
        # Groq exposes GPT-OSS through its OpenAI-compatible API, but LiteLLM
        # must still receive the explicit Groq provider prefix.
        model = model.removeprefix("groq/")
        model = model.removeprefix("openai/")
        return f"groq/openai/{model}"
    ...
    if provider == "azure_foundry" and _is_azure_openai_foundry_endpoint(
        config.api_base, model
    ):
        if model.startswith(("azure/", "azure_ai/")):
            return model
    ...
    known_prefixes = (
        "openrouter/", "anthropic/", "gemini/", "deepseek/", "groq/",
        "azure/", "azure_ai/", "ollama/", "ollama_chat/", "openai/",
    )
...
async def check_llm_health(
    ...
    *,
    ...
    try:
        ...
        if (config.provider or "").strip().lower() == "groq":
            kwargs["model"] = model_name
            kwargs["custom_llm_provider"] = "groq"
        api_version = _azure_foundry_api_version(config)
        if api_version:
            kwargs["api_version"] = api_version
        # Groq GPT-OSS accepts only low/medium/high for reasoning_effort.
        # Never send the app's OpenAI-specific "minimal" value to Groq.
        if (config.provider or "").strip().lower() == "groq":
...
async def complete(
    ...
    try:
        ...
        # Groq GPT-OSS accepts only low/medium/high. Do not forward
        # OpenAI-specific "minimal" to Groq.
        if (config.provider or "").strip().lower() == "groq":
            if config.reasoning_effort in {"low", "medium", "high"}:
                kwargs["reasoning_effort"] = config.reasoning_effort
        elif config.reasoning_effort:
...
async def complete_json(
    ...
    for attempt in range(retries + 1):
        try:
            ...
            elif (config.provider or "").strip().lower() == "groq":
                # Groq GPT-OSS accepts only low/medium/high. The application's
                # OpenAI GPT-5 migration may contain "minimal"; never forward
                # that value to Groq.
