...
def resolve_api_key(stored: dict, provider: str) -> str:
    ...
    # 1. Explicit saved key
    api_key = stored.get("api_key", "")
    if isinstance(api_key, str):
        api_key = api_key.strip()

    # 2. Provider-specific saved key
    if not api_key:
        api_keys = stored.get("api_keys", {})
        if isinstance(api_keys, dict):
            key_name = _PROVIDER_KEY_MAP.get(provider, provider)
            api_key = api_keys.get(key_name, "")
            if isinstance(api_key, str):
                api_key = api_key.strip()

    # 3. Environment provider key
    if not api_key:
        import os
    ...
    # 4. Existing application-level fallback
...
def get_model_name(config: LLMConfig) -> str:
    """Convert provider/model to LiteLLM model format."""
    ...
    # GROQ
    # ---------------------------------------------------------
    # Groq's actual model ID:
    #   openai/gpt-oss-120b
    #
    # LiteLLM MUST receive:
    #   groq/openai/gpt-oss-120b
    #
    # Never allow the generic openai/ prefix check to bypass
...
def _build_router(config: LLMConfig) -> Router:
    """Build a single reliable LLM deployment from the main application config."""
    ...
    # IMPORTANT:
    # Force LiteLLM to use the selected provider.
    # This prevents Groq models such as
    # openai/gpt-oss-120b from accidentally going through
    ...
    logging.info(
        "LLM Router configured: provider=%s model=%s litellm_model=%s",
        provider,
        config.model,
